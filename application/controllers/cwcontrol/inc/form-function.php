...
echo text_area($value,$key);
        }elseif($value["format"]=="textbox"){
echo textbox($value,$key);
...
echo map_form($value,$key);
        }elseif($value["format"]=="gallery_sort"){
echo gallery_list_sort($value,$key);
        }
...
function textbox($filter,$id){
    $label=$filter["label"];
    $name=$filter["name"];

    $label_width=125;
    if(isset($filter["label_width"])){
        $label_width=$filter["label_width"];
    }
    ...
    if(isset($filter["unit"])){
         $unit = '<span class="input-group-addon" style="min-width: 60px;text-align: left;">'.$filter["unit"].'</span>';
    }else{
        $unit='';
    }
    
    if(isset($filter["row"])){
         $rowstart = '<div class="row" style="margin-bottom: '.$filter["row"].'px;">';
         $rowEnd = '</div>';
    }else{
        $rowstart='';
        $rowEnd='';
    ...
    $theme =$rowstart.'
     ...
     <div class="'.$grid.'" id="'.$name.'-'.$id.'">  
              <div class="input-group">
                    <span class="input-group-addon" style="min-width: '.$label_width.'px;text-align: right;">'.$label.'</span>
                    <input class="form-control" name="'.$name.'" id="'.$id.'" type="'.$type.'" value="'.$value.'">
                    '.$unit.'
                </div>  
      </div>
...
function gallery_list_sort($filter,$id){
    $label=$filter["label"];
    $name=$filter["name"];
    $limit="";
    $multiple="";
...
$('#".$id."').filer({
    ...
    afterShow: function(){},
    templates: {
            box: '<ul class=\"jFiler-items-list jFiler-items-grid\" id=\"sortable\"></ul>',
            item: '<li class=\"jFiler-item img-container\">                        <div class=\"jFiler-item-container\">                            <div class=\"jFiler-item-inner\">                                <div class=\"jFiler-item-thumb\">                                    <div class=\"jFiler-item-status\"></div>                                    <div class=\"jFiler-item-info\">                                        <span class=\"jFiler-item-title\"><b title=\"{{fi-name}}\">{{fi-name | limitTo: 25}}</b></span>                                        <span class=\"jFiler-item-others\">{{fi-size2}}</span>                                    </div>                                    {{fi-image}}                                </div>                                <div class=\"jFiler-item-assets jFiler-row\">                                    <ul class=\"list-inline pull-left\">                                        <li><select type=\"text\" name=\"".$id."_sort_item[]\" class=\"form-control sort_item_add\" ".$hidden_text." id=\"sort_item\" ></select>{{fi-progressBar}}</li>                                    </ul>                                    <ul class=\"list-inline pull-right\">                                        <li><a class=\"icon-jfi-trash jFiler-item-trash-action\"></a></li>                                    </ul>                                </div>                            </div>                        </div>                    </li>',
            itemAppend: '<li class=\"jFiler-item\">                            <div class=\"jFiler-item-container\">                                <div class=\"jFiler-item-inner\">                                    <div class=\"jFiler-item-thumb\">                                        <div class=\"jFiler-item-status\"></div>                                        <div class=\"jFiler-item-info\">                                            <span class=\"jFiler-item-title\"><b title=\"{{fi-name}}\">{{fi-name | limitTo: 25}}</b></span>                                            <span class=\"jFiler-item-others\">{{fi-size2}}</span>                                        </div>                                        {{fi-image}}                                    </div>                                    <div class=\"jFiler-item-assets jFiler-row\">                                        <ul class=\"list-inline pull-left\">                                            <li><span class=\"jFiler-item-others\">{{fi-size}}</span></li>                                        </ul>                                        <ul class=\"list-inline pull-right\">                                            <li><a class=\"icon-jfi-trash jFiler-item-trash-action\" idg=\"{{fi-name}}\"></a></li>                                        </ul>                                    </div>                                </div>                            </div>                        </li>',
            progressBar: '<div class=\"bar\"></div>',
            itemAppendToEnd: false,
            removeConfirmation: true,
            _selectors: {
        ...
        ";
if($filter["data_list"]){
$numoption = $filter["data_list"]["num_row"];

foreach ($filter["data_list"] as $key => $val) {
$gallery_Sort1 = trim($val["gallery_Sort"]);
if($val["url"]){        
    ...
    for($i=1;$i<=$numoption;$i++){
        ...
        if($gallery_Sort1 == $i){
    $optionSel .= "<option value=\"".$i."\" data-test=\"".$gallery_Sort1."\" selected >".$i."</option>";
        }else{
    $optionSel .= "<option value=\"".$i."\" data-test=\"".$gallery_Sort1."\" >".$i."</option>"; 
        }
    }
    $size = "size: '<input type=\"hidden\" name=\"gallery_ID[]\" value=\"".$val["id"]."\"/><select class=\"form-control\" id=\"sort_item\" name=\"gallery_Sort[]\">".$optionSel."</select>',";
    $type = substr($val["url"],strrpos($val["url"],".")+1); 
    $type_file = "";
    switch ($type) {
        case 'jpg':
